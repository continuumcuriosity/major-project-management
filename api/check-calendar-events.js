...
// (either already in the review queue, or already linked to an entry),
// and drops genuinely new ones into calendar_review_queue as 'pending'.
...
export default async function handler(req, res) {
  try {
    ...
    // Step 3: check which google_event_ids we already have —
    // both from the review queue AND from entries already linked to Calendar
    const [queueRes, entriesRes] = await Promise.all([
      fetch(`${process.env.SUPABASE_URL}/rest/v1/calendar_review_queue?select=google_event_id`, {
        headers: {
          apikey: process.env.SUPABASE_SERVICE_KEY,
          Authorization: 'Bearer ' + process.env.SUPABASE_SERVICE_KEY
        }
      }),
      fetch(`${process.env.SUPABASE_URL}/rest/v1/entries?calendar_event_id=not.is.null&select=calendar_event_id`, {
        headers: {
          apikey: process.env.SUPABASE_SERVICE_KEY,
          Authorization: 'Bearer ' + process.env.SUPABASE_SERVICE_KEY
        }
    ...
    const existingIds = new Set([
      ...queueRows.map(r => r.google_event_id),
      ...entryRows.map(r => r.calendar_event_id)
    ]);
